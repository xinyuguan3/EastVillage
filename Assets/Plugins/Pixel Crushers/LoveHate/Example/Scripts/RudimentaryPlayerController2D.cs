		...
		protected IEnumerator UpdateUICoroutine()
		{
			while (true)
			{
				if (interactionUI != null && currentTarget != null)
				{
					...
					interactionUI.npcNameText.text=currentTarget.GetName();
					interactionUI.talkCharacterNameText.text=currentTarget.GetName();
					interactionUI.npcImg.sprite=currentTarget.GetImage();
				}
